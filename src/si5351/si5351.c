...
/*
 * vfo.cpp - Si5351 library for Arduino
 *
 * Copyright (C) 2015 Jason Milldrum <milldrum@gmail.com>
 *                    Dana H. Myers <k6jq@comcast.net>
 *
 * Some tuning algorithms derived from clk-vfo.c in the Linux kernel.
...
struct Si5351Variables vfo;

static uint64_t Si5351_pll_calc(uint64_t, struct Si5351RegSet *, int32_t);
static uint64_t Si5351_multisynth_calc(uint64_t, uint64_t, struct Si5351RegSet *);
...
void Si5351_init(uint8_t xtal_load_c, uint32_t ref_osc_freq)
{
	...
	vfo.lock_plla = SI5351_CLKNONE;
	vfo.lock_pllb = SI5351_CLKNONE;
	vfo.clk0_int_mode = 0;
	vfo.clk1_int_mode = 0;
	vfo.clk2_int_mode = 0;
	vfo.plla_freq = 0ULL;
	...
	vfo.clk1_freq = 0ULL;
	vfo.clk2_freq = 0ULL;
	vfo.xtal_freq = SI5351_XTAL_FREQ;
...
uint8_t Si5351_set_freq(uint64_t freq, uint64_t pll_freq, enum si5351_clock clk)
{
	...
	else
	{
		...
		case SI5351_CLK1:
			...
			if(vfo.lock_pllb == SI5351_CLK2)
			{
				...
				if((freq >= SI5351_MULTISYNTH_SHARE_MAX * SI5351_FREQ_MULT
					|| freq < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 128) && freq != vfo.clk2_freq)
				{
					vfo.clk1_freq = 0;
					return 1;
				}
			...
			{
				vfo.lock_pllb = SI5351_CLK1;

				// Recalc and rewrite the multisynth parameters on CLK2
				if(vfo.clk2_freq != 0)
				{
					struct Si5351RegSet ms_temp_reg;
					r_div = Si5351_select_r_div(&vfo.clk2_freq);
					Si5351_multisynth_calc(vfo.clk2_freq, vfo.pllb_freq, &ms_temp_reg);
		...
		case SI5351_CLK2:
			...
			if(freq >= SI5351_MULTISYNTH_SHARE_MAX * SI5351_FREQ_MULT || freq < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 128)
			{
				vfo.lock_pllb = SI5351_CLK2;

				if(vfo.clk1_freq != 0)
				{
					// Recalc and rewrite the multisynth parameters on CLK1
					struct Si5351RegSet ms_temp_reg;
					r_div = Si5351_select_r_div(&vfo.clk1_freq);
					Si5351_multisynth_calc(vfo.clk1_freq, vfo.pllb_freq, &ms_temp_reg);
...
/*
 * Si5351_set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
 *
...
void Si5351_set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
{
  ...
  Si5351_pll_calc(pll_freq, &pll_reg, vfo.ref_correction);
 ...
 * the vfo. Once this calibration is done accurately, it
...
void Si5351_set_int(enum si5351_clock clk, uint8_t enable)
{
	...
	{
	case SI5351_CLK0:
		vfo.clk0_int_mode = enable;
		break;
	case SI5351_CLK1:
		vfo.clk1_int_mode = enable;
		break;
	case SI5351_CLK2:
		vfo.clk2_int_mode = enable;
		break;
	default:
		break;
	}
...
uint8_t Si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	ret=mchf_hw_i2c_WriteBlock(SI5351_BUS_BASE_ADDR, addr, data, (ulong) bytes);
...
uint8_t Si5351_write(uint8_t addr, uint8_t data)
{
	...
	ret=mchf_hw_i2c_WriteRegister(SI5351_BUS_BASE_ADDR,addr,data);
...
uint8_t Si5351_read(uint8_t addr)
{
	...
	ret=mchf_hw_i2c_ReadRegister(SI5351_BUS_BASE_ADDR,addr, &reg_val);
...
static uint64_t Si5351_pll_calc(uint64_t freq, struct Si5351RegSet *reg, int32_t correction)
{
	uint64_t ref_freq = vfo.xtal_freq * SI5351_FREQ_MULT;
	...
	ref_freq += (uint32_t)((correction / 10000000.0) * ref_freq);
